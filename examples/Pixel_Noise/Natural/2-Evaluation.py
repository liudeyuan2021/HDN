if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        img = cv2.resize(img, (img.shape[0]*2//3, img.shape[1]*2//3))
        cv2.imwrite(f'/home/taichigraphics/Desktop/Sensetime/HDN/dataset/png_test/{i:03d}.png', img)

        img = np.array(img, dtype=np.float32)
        yuv = cv2.cvtColor(img, cv2.COLOR_BGR2YUV)
        y = yuv[:, :, 0]
        y_mmse, samples = boilerplate.predict(y, num_samples, model, gaussian_noise_std, device, tta)
        y_mmse = np.clip(y_mmse, 0, 255)
        yuv[:, :, 0] = y_mmse
        result = cv2.cvtColor(yuv, cv2.COLOR_YUV2BGR)
        result = np.array(result, dtype=np.uint8)
        cv2.imwrite(f'/home/taichigraphics/Desktop/Sensetime/HDN/dataset/png_test/{i:03d}_result.png', result)
